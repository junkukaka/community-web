<template>
  <v-card flat class="pa-2">
    ...
    <v-dialog v-model="commmentDialog" width="700">
      <v-card flat>
        <v-card-title
          v-text="commentList.length > 0 ? 'Comments list' : '댓글 없습니다.'"
          ...
          <template v-for="(item, index) in commentList">
            ...
            <v-list-item v-else :key="`item-${index}`">
              ...
              <v-list-item-content>
                ...
                <v-list-item-subtitle style="white-space: pre-line">{{ item.content }}</v-list-item-subtitle>
              </v-list-item-content>
              ...
              <v-list-item-action>
                ...
                <v-list-item-subtitle>
                  <v-btn text @click="deleteComment(item.id)">
                    <v-icon small> mdi-delete </v-icon>
                  </v-btn>
                </v-list-item-subtitle>
              </v-list-item-action>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: "Name", value: "memberName", align: "center" },
  ...
  methods: {
    initialize() {
      ...
      this.$nextTick(function () {
        this.$http
          .get("/adminArticle/community/selectCommunities", request)
          .then((response) => {
    ...
    getComments(id) {
      this.commentList = [];
      this.$nextTick(function () {
        this.$http.get(`/comment/comments/${id}`).then((response) => {
    ...
    deleteItem(id) {
      this.deletedId = id;
      this.deleteFlag = "COMMUNITY";
      this.dialogDelete = true;
    },

    deleteItemConfirm() {
      if (this.deleteFlag == "COMMUNITY") {
        this.$http
        ...
        this.$http
          .delete(`/comment/comments/${this.deletedId}`)
          .then((response) => {
            if (response.data.code == 200) {
              this.closeDelete();
