<template>
  <v-card flat class="pa-2">
    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>{{$t('my00',{0:$t('collect')})}}</v-toolbar-title>
    ...
    <template v-slot:[`item.title`]="{ item }">
       <div>
        <router-link :to="`/community/communityDetail?id=${item.communityId}`" color="indigo" 
          v-text="
            `${
  ...
  methods: {
    ...
    initialize() {
      ...
      this.$nextTick(function () {
        this.$http
          .get("/comInfo/collect/collectPageList", request)
          .then((response) => {
    ...
    deleteItemConfirm() {
      this.$http.post('/comInfo/save/collect',this.deleted)
      .then((response) => {
